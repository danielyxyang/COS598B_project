...
if ENV == COLAB:
  ...
  parser.add_argument("-b", "--batch", help="specify experiment batch number")
  parser.add_argument("--dev", action="store_true")
  
  args = parser.parse_args()
  OUTPUT_DIR = args.out
  EXPERIMENT_BATCH_ID = args.id
  DEV = args.dev
...
class DataShapleySolver():
  ...
  def _utility_wrapper(self, subset_indices, statistics=True):
    if statistics:
  ...
  def _compute_shapley_with_subsets(self):
    ...
    for S in tqdm(subsets, total=n_subsets, desc="Loop 1"):
      ...
      for i in tqdm(not_S, desc="Loop 2", mininterval=0.5, disable=len(not_S) < 500, leave=False):
        self.shapley_values[i] += (self._utility_wrapper(list(S) + [i]) - self._utility_wrapper(list(S))) / factor
    # finish computation
    self.utility_eval_counter_pbar.close()

  def _compute_shapley_with_permutations(self, permutations, n_permutations):
    # initialize computation
    self._reset(n_evals=n_permutations * self.dataset.size)
    # compute marginal utilities over given permutations
    for permutation in tqdm(permutations, total=n_permutations, desc="Loop 1"):
      ...
      for end in tqdm(range(len(permutation)), desc="Loop 2", mininterval=0.5, disable=len(permutation) < 500, leave=False):
        ...
        utility = self._utility_wrapper(list(S) + [i])
...
if DEV:
  ...
  cmap = {1: "blue", -1: "red"}
  dataset = GaussianDataset(gaussians, cmap, sizes=10)
  dataset.plot()
...
class GaussianExperiment():
  ...
  def run(self, save_results=True):
    ...
    if save_results:
      ...
      folder = os.path.join(OUTPUT_DIR, "results", self.experiment_type, self.experiment_name)
...
def get_experiment_batch(e, test_run=False):
  return [
      GaussianExperiment(
          ...
          seed=seed,
          tag=e["tag"] if not test_run else "dev",
...
def run_experiment_batch(experiments, batch_id="unspecified"): 
...
experiments1 = { # 12 experiments, exact, different seeds
    "tag": "size10_exact",
    "model_constructors": {
...
experiments2 = { # 12 computations, approx, different num_permutations
    "tag": "size100_approx",
    "model_constructors": {
        "LogisticRegression": lambda: pipeline.make_pipeline(preprocessing.StandardScaler(), linear_model.LogisticRegression()),
        "LinearSVC": lambda: pipeline.make_pipeline(preprocessing.StandardScaler(), svm.LinearSVC()),
...
experiments_dev = {
    "tag": "dev",
    "model_constructors": {
        # "LogisticRegression1": lambda: linear_model.LogisticRegression(),
        # "LogisticRegression2": lambda: pipeline.make_pipeline(preprocessing.StandardScaler(), linear_model.LogisticRegression()),
        # "MLPClassifier1": lambda: neural_network.MLPClassifier(hidden_layer_sizes=(10, 10), solver='lbfgs', random_state=1),
        "MLPClassifier2": lambda: pipeline.make_pipeline(preprocessing.StandardScaler(), neural_network.MLPClassifier(hidden_layer_sizes=(10, 10), solver='lbfgs', random_state=1)),
        # "LinearSVC1": lambda: svm.LinearSVC(),
        # "LinearSVC2": lambda: pipeline.make_pipeline(preprocessing.StandardScaler(), svm.LinearSVC()),
        # "SVC_rbf": lambda: svm.SVC(kernel='rbf', gamma=0.7),
    },
    "datasets": [
        dataset_gaussian1,
    ...
    "params": [
        ...
        {"train_dataset_size": 10, "test_dataset_size": 1000, "num_permutations": 100},
    ],
    "seeds": [
        0,
    ],
...
experiment_batches = [
    get_experiment_batch(experiments_dev), # index 0 should always be dev experiments
    get_experiment_batch(experiments1, test_run=DEV),
    get_experiment_batch(experiments2, test_run=DEV),
]

if EXPERIMENT_BATCH_ID is None:
  # run all experiment batches
  for i, batch in enumerate(experiment_batches):
    run_experiment_batch(batch, batch_id=i)
else:
  # run only experiments of specified batch
  run_experiment_batch(experiment_batches[EXPERIMENT_BATCH_ID], batch_id=EXPERIMENT_BATCH_ID)

# !zip results/GaussianExperiment_size8_exact.zip results/GaussianExperiment_size8_exact/*/*
